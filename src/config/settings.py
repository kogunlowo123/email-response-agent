"""Application configuration for Email Response Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="email-response-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/email_response_agent",
    ...
    opensearch_index: str = Field(default="email_response_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="email-response-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="email-response-agent", description="A2A agent ID")
